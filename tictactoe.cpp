//tictactoe - 2 player game of tictactoe in the console
//Emily MacPherson, 10/11/21  
...
int main()
{
  char grid[3][3];
  bool running = true;
  char coords[1000]; //1000 is just an arbitrary number that we can assume the user probably wont input more than, not foolproof but good enough for this hehehe 

  newgame(grid);
  while(running == true)
  { 
    cout << turn << "'s turn. Enter coords (letter first):" << endl;
    cin.get(coords, 1000); //If I just take the first 2 chars input like a362 would put it at a3, which isnt necessarily bad but it annoys me :D
    cin.get();
    bool validcoords = false;
    while(validcoords == false)
	  {
      //coords too long or invalid
      if((strlen(coords) > 2) || (coords[0] != 'a' &&coords[0] != 'b' && coords[0] != 'c') || (coords[1] != '1' && coords[1] != '2' && coords[1] != '3'))
      {
